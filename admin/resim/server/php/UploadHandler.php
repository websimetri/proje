...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
		...
		$klasor = $_SESSION['resim_id'];
		
        $this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/'.$klasor.'/',
            'upload_url' => $this->get_full_url().'/files/'.$klasor.'/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function trim_file_name($name,
        ...
        //$name = trim(basename(stripslashes($_SESSION['urun_sanal_id']."_".$randvar.$name)), ".\x00..\x20");
	 ...
	 //settype($randvar,"string"); $name = trim(basename(stripslashes($_SESSION['urun_sanal_id']."_".$randvar)), ".\x00..\x20");        // Add missing file extension for known image types:
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
			...
			// resim ekleme işlemi
			//$urun_id = mysql_fetch_array(mysql_query("select id from urunler where ref_kodu = '".$_SESSION['urun_sanal_id']."'"));
			//$yaz = mysql_query("INSERT INTO urun_resimleri (id ,urun_id,adi,klasor)VALUES (NULL , '".$_SESSION['resim_id']."',  '".$file->name."' , '".$_SESSION['resim_id']."')");
                    $dns = 'mysql:host=localhost;dbname=bulut';
                    $kullaniciAdi = "root";
                    $sifre = "";
                    try {
    ...
    public function delete($print_response = true) {
        ...
        foreach($file_names as $file_name) {
			...
			//resim silme işlmei		
			//$urun_id = mysql_fetch_array(mysql_query("select id from urunler where ref_kodu = '".$_SESSION['urun_sanal_id']."'"));
			//mysql_query("delete from urun_resimleri where urun_id = '".$_SESSION['resim_id']."' and  adi = '".$file_name."' limit 1");
                    $dns = 'mysql:host=localhost;dbname=bulut';
                    $kullaniciAdi = "root";
                    $sifre = "";
                    try {
                        $db = new PDO($dns, $kullaniciAdi, $sifre);
                        $db -> exec("SET CHARACTER SET utf8");
                        $kul = $db->query("delete from urun_resimleri where urun_id = '".$_SESSION['resim_id']."' and  adi = '".$file_name."' limit 1");
                    }catch(Exception $e) {
                        echo "PDO Bağlantı Hatası : ". $e->getMessage();
                    }
