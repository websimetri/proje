...
class ResimIslemleri
{
	 ...
	 * @return int|string
	 */
	public static function imageUpload($inputname, $imageResize = false)
	{
				...
				if ($durum) {
					...
					if (!getimagesize($_FILES[$inputname]["tmp_name"]) & is_executable($_FILES[$inputname]["tmp_name"])) {
						...
						if (copy($path, $ciktiYolu)) {
							if (is_array($imageResize) && $imageResize != false) {
								if (ResimIslemleri::imageResize($ciktiYolu, $klasoryolu . "/" . $dosyaAdi, $imageResize)) {
									return $ciktiYolu;
								} else {
